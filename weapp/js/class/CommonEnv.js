...
var _default = {
  alert: function alert(params, callback) {
    this.fakeAlert(params.title, function (ret) {
      if (ret) {
        callback && callback({
          btnIndex: 0
        });
  ...
  authorize: function authorize(scope) {
  ...
  downloadFile: function downloadFile(url, filePath) {
    return new Promise(function (r, j) {
      wx.downloadFile({
        url: url,
        // 仅为示例，并非真实的资源
  ...
  put2Desktop: function put2Desktop() {
    var _this = this;
    ...
    return new Promise(function (r, j) {
      ...
      return _this.awaitime(3000);
    });
  },
  comfirm: function comfirm(title, content) {
    return new Promise(function (r, j) {
      wx.showModal({
        title: title,
        content: content,
  ...
  judgeIsInDouYin: function judgeIsInDouYin() {
    return navigator.userAgent.toLowerCase().includes("toutiaomicroapp");
  },
  judgeIsInQQ: function judgeIsInQQ() {
    return this.getUrlParam('appName') == 'QQ';
  },
  judgeisInToutiao: function judgeisInToutiao() {
    return this.getUrlParam('appName') == 'Toutiao';
  },
  judgeIsInMiniProgram: function judgeIsInMiniProgram() {
    return /miniProgram/i.test(navigator.userAgent.toLowerCase());
  },
  isQQMiniApp: function isQQMiniApp() {
    var runtime = typeof qq === "undefined" ? "wechat" : "qq";
    return runtime == 'qq';
  },
  isWXMiniApp: function isWXMiniApp() {
    var runtime = typeof qq === "undefined" ? "wechat" : "qq";
    return runtime == 'wechat';
  },
  awaitime: function awaitime(time) {
    return new Promise(function (r, j) {
      setTimeout(function () {
        r();
      }, time);
  ...
  gotoWeixinPay: function gotoWeixinPay(orderData, callback) {
    var _this2 = this;

    console.log('debug:gotoWeixinPay');

    var func = function func() {
      _this2.onBridgeReady(orderData, callback);
    };
    ...
    return this.awaitime(1).then(function () {
      return orderData;
    });
  },
  judgeIsInWeixin: function judgeIsInWeixin() {
    var isInWeixin = /micromessenger/i.test(navigator.userAgent.toLowerCase());
    return isInWeixin;
  },
  onBridgeReady: function onBridgeReady(payData, callback) {
    var taht = this;
  ...
  wxWapPay: function wxWapPay(redirect, params) {},
  ...
  fakeAlert: function fakeAlert(msg, callback, sureText, cancelText) {
    ...
    dialog_msg2.innerHTML = msg; // var dialog_cancel = document.getElementById("dialog_cancel");
